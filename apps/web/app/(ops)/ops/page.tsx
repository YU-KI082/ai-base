  ...
  return (
    <main className="animate-in">
      <div className="page-header">
        ...
        <Link className="btn btn-primary" href="/ops/company">
          {locale === "ja" ? "AI会社" : "AI Company"}
        </Link>
        <Link className="btn btn-ghost" href="/ops/ingest">
        ...
        <section className="card-surface" style={{ padding: "1.15rem" }}>
          ...
          {pending.length === 0 ? (
            ...
            <ul style={{ margin: 0, paddingLeft: "1.1rem" }}>
              {pending.map((d) => (
                <li key={d.id}>
                  <Link href={`/ops/drafts/${d.id}`}>{d.id}</Link>
                </li>
              ))}
            </ul>
          )}
